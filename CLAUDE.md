...
instructions live in `src/jobpilot-skills/` (canonical source). Provider
plugins at `src/jobpilot-claude-plugin/` and `src/jobpilot-codex-plugin/`
hold a manifest + `.mcp.json`; their `skills/` subtrees are **generated** by
`scripts/sync-skills.ts` (runs automatically via `predev` / `prestart`) and
are gitignored. A companion .NET process, **JobPilot.Terminal** at
`src/JobPilot.Terminal/` (port 8001), hosts one active provider PTY so the
web UI can embed an interactive terminal and inject provider-specific
commands.

## Architecture

- **Canonical skills pack** in `src/jobpilot-skills/` owns provider-neutral
  workflows (`skills/<name>/SKILL.md`) and shared instructions
  (`shared/*.md`). Cross-skill references in the body use
  `<skill-name>-command` placeholders; references to shared files use the
  `${JOBPILOT_SKILLS_ROOT}` env-var path. The generator substitutes both per
  provider.
- **Generator** at `scripts/sync-skills.ts` reads each canonical SKILL.md and
  writes provider-specific copies into each plugin's `skills/` directory.
  For Claude: keeps full frontmatter (including `allowed-tools`), substitutes
  `<X-command>` → `/jobpilot:X` and `${JOBPILOT_SKILLS_ROOT}` →
  `${CLAUDE_PLUGIN_ROOT}/../jobpilot-skills`. For Codex: prefixes the skill
  `name` with `jobpilot-`, strips Claude-only frontmatter, substitutes
  `<X-command>` → `$X`, leaves `${JOBPILOT_SKILLS_ROOT}` as the
  terminal-exported env var. Both plugin `skills/` directories are
  gitignored and regenerated by `bun run sync-skills` (also runs as
  `predev`/`prestart`).
- **Claude Code plugin** in `src/jobpilot-claude-plugin/` owns
  `.claude-plugin/plugin.json` and `.mcp.json`. Namespace is `jobpilot`, so
  skills run as `/jobpilot:<skill>`.
- **Codex plugin** in `src/jobpilot-codex-plugin/` owns
  `.codex-plugin/plugin.json` and `.mcp.json`. Skills run as `$<skill>`
  (e.g., `$apply`, `$autopilot`).
...
- **Skills talk to the web app over HTTP.** They do not own persistence.
  ...
  a clear message.
- **Humanizer** lives in `src/jobpilot-skills/skills/humanizer/SKILL.md`,
  chained from `cover-letter` and `upwork-proposal` via the
  `<humanizer-command>` placeholder.

## Key Patterns

- Edit canonical skills at `src/jobpilot-skills/skills/<name>/SKILL.md` only;
  never touch the generated plugin `skills/` trees. Run
  `bun run sync-skills` after editing.
- `src/jobpilot-skills/shared/setup.md` is the single source of truth for loading profile,
  resume, and credentials. Reference it as
  `${JOBPILOT_SKILLS_ROOT}/shared/setup.md` in canonical bodies; the
  generator rewrites the path per provider.
...
| `src/jobpilot-skills/skills/<name>/SKILL.md`                                | Canonical provider-neutral JobPilot workflow prompts (edit these).      |
| `scripts/sync-skills.ts`                                                    | Generator: emits per-provider SKILL.md trees from the canonical pack.   |
...
| `src/jobpilot-claude-plugin/skills/`                                        | Generated (gitignored); regenerate with `bun run sync-skills`.          |
...
| `src/jobpilot-codex-plugin/skills/`                                         | Generated (gitignored); regenerate with `bun run sync-skills`.          |
