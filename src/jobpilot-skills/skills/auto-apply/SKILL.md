...
2. Follow `${JOBPILOT_SKILLS_ROOT}/shared/auth.md` — logs in, and **registers a new account when none exists, without asking**.
3. Fill the search fields and submit.
4. Take a `browser_snapshot` narrowed to the results list (per `${JOBPILOT_SKILLS_ROOT}/shared/browser-tips.md`) to read `{ title, company, location, url }` per row.
...
2. **Authentication** — if a login/registration wall appears, follow `${JOBPILOT_SKILLS_ROOT}/shared/auth.md`. On unrecoverable login failure for the domain: POST `/result` `outcome:"failed"`, `failReason:"Login failed for <domain>"`, close tab 2, continue.
3. **Tailor Resume** — invoke `<tailor-resume-command> $DIGEST` (empty → fall back to the job URL). Capture the variant id + PDF URL. No usable base → POST `/result` `outcome:"failed"`, `failReason:"No tailorable resume base"`, close tab 2, continue.
4. **Fill Forms** — follow `${JOBPILOT_SKILLS_ROOT}/shared/form-filling.md`. Upload the tailored variant. Use `autoApply.defaultStartDate`; ask once for salary expectation and remember it for the run.
...
2. **Account handling** — follow `shared/auth.md`: register when missing (without asking), forgot-password via `<get-code-command>` when stale.
3. **Never process payments** — POST `/result` `outcome:"failed"`, `failReason:"Payment required"`.
4. **Email codes** — fetch automatically via `<get-code-command> "<board-domain>"` (see `auth.md`); only ask the user when it returns nothing. **CAPTCHAs / 2FA** — pause and ask. One-time per board, not per-job failures.
