...
7. **4.7 Record Result** — POST `/api/runs/$RUN_ID/jobs/<key>/result` with `outcome:"applied"`/`"failed"`/`"skipped"`. Atomic: updates the Job, creates Application, marks queue consumed, recomputes summary.
