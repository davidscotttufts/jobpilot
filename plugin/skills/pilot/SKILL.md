...
- **`job`** → **first check whether this is a crash-recovery question**, i.e. the answer is one of `It was submitted - mark applied` / `It was not submitted - try again`. Those two never re-apply blindly - the job is held precisely because nothing can tell a sent application from an unsent one, and delegating a worker would send the duplicate the hold exists to prevent:
  - `It was submitted - mark applied` → **do not open a browser.** Record the application that was never recorded: `POST /api/campaigns/$CID/jobs/$KEY/result` with `{"outcome":"applied","appliedAt":"<now>"}`. Journal that the outcome came from the user, not from an observed submit.
  - `It was not submitted - try again` → release the hold, then apply normally: `PATCH /api/campaigns/$CID/jobs/$KEY` with `{"status":"approved","confirmNotSubmitted":true}` (the server refuses `approved`/`applying` on a held job without it, and this is the only flag that clears it), then delegate `job-worker` as `job.apply` below.
  - Free text → interpret it as one of the two; if it genuinely cannot be read either way, journal that and leave the job held for the next answer.

  Otherwise it is an ordinary mid-apply question: delegate `job-worker` apply mode as `job.apply` above with `answer` included in its input as `providedAnswers` (pre-provided user answers the worker reads instead of asking again); record the result exactly as `job.apply`. **When the question was a pre-submit review (`kind:"approval"`) and the user approved, re-delegate with `preSubmitReview:false`** - the form is already filled, so the worker confirms and submits. Leaving it true refills and holds again, and because a held application creates no Application row, the review window never closes: the job would be held forever.
...
`maxApplications:$maxApps`): it is the cap the user set on the campaign this search repeats, and
dropping it would silently turn a capped weekly run into an unlimited one.
...
**Search stewardship.** A search with `cadence:"weekly"` is pinned by the user: never change its `cadence`, `cadenceDays`, `cadenceHour`, or `cadenceTimeZone`, and never delete it - they asked for that day, and a rewrite that drops the pin reads to them as the schedule silently breaking. Its `query`/`board` stay editable, and the pin survives the edit. Otherwise, when the diagnosis implicates the search itself - fundamentally dry or mistargeted, not merely campaign tuning - make at most ONE search change per cycle, instead of or alongside config tuning: `POST`/`PATCH`/`DELETE $JOBPILOT_API/api/pilot/searches[/:id]` (`GET /api/pilot/searches` for ids). Update `reason` to say why, and journal the change.
...
1. **One item, one worker, one cycle** - unless `budget.maxConcurrentApplies` is above 1. Then you
   ...
   own browser profile**; two workers sharing one profile do not just serialize, they collide - a
   locked profile was the single largest cause of failed applies. Lease one per claim rather than
   assigning by convention:

```bash
curl -fsS -X POST -H "authorization: Bearer $JOBPILOT_API_TOKEN" \
  -H 'content-type: application/json' -d '{"server":"playwright-2"}' \
  "$JOBPILOT_API/api/pilot/claims/$CLAIM_ID/browser"
```

   `budget.browsersInUse` lists what is already held, so pick a name from `plugin/.mcp.json` that
   is not in it. A `409` means someone took it first - pick another, or run this item alone. Pass
   the leased name to the worker as `browserServer`. The lease ends with the claim, including when
   the claim expires under a crashed worker, so nothing has to be released by hand. Fan out only
   as far as there are servers configured, however high the limit is. The host loops, not you.
