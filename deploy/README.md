...
their own machines - nothing agent-related is deployed here.
...
- [docker-compose.yml](docker-compose.yml) - pulls `api` and `web` images from
  ...
  localhost only.
- [jobpilot.conf](jobpilot.conf) - nginx site config: one origin, web at `/`,
  API proxied at `/api` with SSE buffering off. Install steps are in the file's
  header comment; TLS comes from `certbot --nginx`.
- `.env` (not committed) - `DATABASE_URL` for the external PostgreSQL,
